...
async def recive_file(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    file_path = DIR+f'\{update.message.from_user.id}'+f'\{file_name}.'+'{}'
    credentials_path = DIR+"\credentials.json"
    print(file_format)

        
    if file_format == 'pdf':
        file_path_pdf = DIR+f'\{update.message.from_user.id}'+f'\{file_name}'+'{}.{}'
        print('ok')
        with open(file_path.format('pdf'),'wb') as f:
            f.write(file)
        count = extract_png(file_path.format('pdf'), DIR+f'\{update.message.from_user.id}'+f'\{file_name}')
